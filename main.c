 ...
 *
 * @author      [Chunyang Mou, Chengxuan Cai]
 *
...
#define RELAY_NODE_ADDR     "fe80::212:4b00:18e0:ba41" //teacher
...
int main(void)
{
    ...
    udp_rx_args.udp_port = UDP_RX_PORT;
...
#endif
        char buff[30];
        if (sample > 300 && sample < 1024){
           strcpy(buff, "moucai/light/: LIGHTS_ON");
        }
        else if(sample < 300 && sample > 0){
           strcpy(buff, "moucai/light/: LIGHTS_OFF");
        }
        else if (sample == 1024){
            strcpy(buff, "moucai/light/: FAKE_DATA");
        }
        udp_send(RELAY_NODE_ADDR, UDP_SEND_PORT_STR, buff);
        mutex_unlock(&mutex);
